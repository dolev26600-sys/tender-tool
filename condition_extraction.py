...
def extract_conditions(pdf_text: str, *, source_hint: Optional[str] = None) -> dict:
    ...
    חיסכון בעלות: מסמן את הטקסט המלא של המכרז ב-cache_control, כדי שאם
    verify_conditions() נקראת אחר כך על אותו pdf_text (וזה תמיד קורה - זה
    בדיוק תהליך הבדיקה העצמית), הקריאות החוזרות ישלמו רק כ-10% ממחיר
    הטוקנים על החלק הזה במקום מחיר מלא.
    """
    ...
    # לא מסמנים כאן cache_control על טקסט המכרז עצמו - קריאה זו רצה פעם
    # אחת בלבד לכל מכרז, כך שלא יהיה למי "לקרוא" את המטמון בחזרה (רק
    # מפסיד את פרמיית הכתיבה). המטמון על system בלבד עדיין משתלם כי הוא
    # זהה בכל מכרז וכל קריאה, אז חוזר על עצמו לאורך זמן.
